#!/bin/bash
# Check if BOTH old.tex and new.tex are missing
if [ ! -f "old.tex" ] && [ ! -f "new.tex" ]; then
    echo "Error: old.tex and new.tex do not exist."
    exit 1
fi

echo "Both files found. Proceeding with diff generation..."
