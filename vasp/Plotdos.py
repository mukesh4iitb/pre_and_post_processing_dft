...
Author: It was originally written by Dr. Uthpala Herath. I have modified it to works with the latest version of pymatgen and added some features.
...
def plot_total(args, sigma_val=0.05):
    """
    ...
    tdos = vasprun.tdos
    plot = DosPlotter(sigma=sigma_val)
    # plot = DosPlotter()
    plot.add_dos("Total DOS", tdos)

    ax = plot.get_plot(xlim=args.xlim, ylim=args.ylim)
    ...
    ax.figure.savefig("dos_total.png", dpi=300)


def plot_dos_atoms(args, sigma_val=0.05):
    """
    ...
    if args.total:
        ax.figure.savefig("dos_atomic_total.png", dpi=300, bbox_inches="tight")
    else:
        ax.figure.savefig("dos_atomic.png", dpi=300, bbox_inches="tight")


def plot_dos_orbitals(args, sigma_val=0.05):
    """
    ...
    if args.atom:
        ...
        ax.figure.tight_layout()
        save_str = f"dos_orbital_{args.atom}.png"
        ax.figure.savefig(save_str, dpi=300, bbox_inches="tight")
        ...
        ax.figure.tight_layout()
        ax.figure.savefig("dos_orbital_total.png", dpi=300, bbox_inches="tight")


def plot_dos_d(args, sigma_val=0.05):
    #     """
    ...
    if args.minmax:
        for count, i in enumerate(range(args.minmax[0], args.minmax[1])):
            data[f"data{count}"] = vasprun.complete_dos.get_site_t2g_eg_resolved_dos(
                vasprun.structures[0][i]
            )
            print(vasprun.structures[0][i])
        ...
        matchlistdic = collections.OrderedDict()
        for count in range(len(matchlist)):
            matchlistdic[matchlist[count][0]] = int(matchlist[count][1])
    ...
    for k in data:
        if k != "data0":
            data_sum_t2g += data[k]["t2g"]
            data_sum_eg += data[k]["e_g"]

    print("\nPlotting d orbital projected DOS...")

    if args.atom:
        str_t2g = f"{args.atom} d-t$_{{2g}}$"
        str_eg = f"{args.atom} d-e$_g$"
        str_save = f"dos_d_{args.atom}.png"
    ...
    ax.figure.tight_layout()
    ax.figure.savefig(str_save, dpi=300, bbox_inches="tight")
